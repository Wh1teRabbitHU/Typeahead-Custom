 ...
 */
 ...
 window.TypeaheadCustom = new function() {
 	var Typeaheads = {};

 	var _typeaheadCustom = function TypeaheadCustom(OPTIONS) {

		// Initialize options
		var target = OPTIONS.target || $(".autocomplete");
		var selector = OPTIONS.selector || ".autocomplete-selector-div";
		var suggestion = OPTIONS.suggestion || ".autocomplete-suggestion";
		var id = OPTIONS.hasID ? OPTIONS.id || ".autocomplete-id" : null;
		...
		var show_empty_list = OPTIONS.show_empty_list !== false;
		...
		this.bindEvents = function() {
			target.bind("keydown", keydownFunction);
			target.bind("keyup", keyupFunction);
			target.bind("blur", blurFunction);
		}

		this.unbindEvents = function() {
			target.unbind("keydown", keydownFunction);
			target.unbind("keyup", keyupFunction);
			target.unbind("blur", blurFunction);
		}
		...
		var select = function (_target) {
			if(getCurrentSelector(_target).children("select").is(":visible")) {
		...
		// Init current typeahead
		this.bindEvents();

		$(selector).children("select").attr("size", suggestion_size);

		target.each(function () {
			adapterMap[$(this).attr("id")] = new _autoCompleteAdapter($(this));
			selectedMap[$(this).attr("id")] = $(this).val() !== 0 || getCurrentId($(this)).val() !== 0;
		});
	}

	var _autoCompleteAdapter = function AutoCompleteAdapter(_target) {
	...
	this.CreateTypeahead = function(name, params) {
 		Typeaheads[name] = new _typeaheadCustom(params);
 	}

 	this.GetTypeahead = function(name) {
 		return Typeaheads[name];
 	}
